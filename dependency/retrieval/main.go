...
func main() {
	...
	releases, err := fetchReleases("watchexec", "watchexec")
	if err != nil {
	...
	for _, release := range releases {
		...
		amd64Asset := findAsset(release.Assets, `watchexec-.+-x86_64-unknown-linux-musl\.tar\.xz`)
		arm64Asset := findAsset(release.Assets, `watchexec-.+-aarch64-unknown-linux-musl\.tar\.xz`)
		...
		sourceURL := fmt.Sprintf("https://github.com/watchexec/watchexec/archive/refs/tags/%s.tar.gz", release.TagName)
		...
		cpe := fmt.Sprintf("cpe:2.3:a:watchexec:watchexec:%s:*:*:*:*:*:*:*", versionStr)
		purlAMD64 := fmt.Sprintf("pkg:generic/watchexec@%s?arch=amd64", versionStr)
		purlARM64 := fmt.Sprintf("pkg:generic/watchexec@%s?arch=arm64", versionStr)
